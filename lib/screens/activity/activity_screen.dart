...
class _ActivityScreenState extends State<ActivityScreen> {
  ...
  static const List<String> _sportNames = [
    'Running',
    'Cycling',
    'Swimming',
    'Weightlifting'
  ...
  int _estimateCalories(int durationMinutes, double weightKg) {
    return HealthCalculator.estimateCalories(
  ...
  int _estimateSteps(int durationMinutes) {
    return HealthCalculator.estimateSteps(
      sportType: _sportNames[_selectedSportIndex],
      durationMinutes: durationMinutes,
    );
  }

  Future<void> _logActivity(String userId, double userWeightKg) async {
    ...
    final cal = _estimateCalories(duration, userWeightKg);
    final steps = _estimateSteps(duration);
  ...
  Widget build(BuildContext context) {
    ...
    final userWeightKg = auth.userProfile?.weight ?? 70.0;
    ...
    // Preview estimate for the currently selected sport + duration typed
    final int? previewDuration = int.tryParse(_durationController.text);
    final int previewCal = previewDuration != null
        ? _estimateCalories(previewDuration, userWeightKg)
        : 0;
    final int previewSteps =
        previewDuration != null ? _estimateSteps(previewDuration) : 0;
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // Duration input — FIX: added prefixIcon
                  TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      helperText: previewDuration != null && previewDuration > 0
                          ? '≈ $previewCal kcal · $previewSteps steps (based on your weight)'
                          : 'Enter duration to preview estimate',
                    ),
                  ),
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: _isSaving
                          ? null
                          : () => _logActivity(userId, userWeightKg),
                      child: _isSaving
                          ? const SizedBox(
                              height: 20,
                              width: 20,
